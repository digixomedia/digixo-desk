/*
# Phase 2B Fix 3: Remove ON CONFLICT from payment insert, fix tags cast

1. core_add_payment: Remove ON CONFLICT clause — the pre-check SELECT
   already handles idempotency. The partial unique index on
   payments.idempotency_key doesn't support ON CONFLICT.
2. core_update_customer: Cast jsonb tags to text[] properly.
*/
...
BEGIN
  ...
  -- Pre-check for existing payment with same namespaced key
  ...
  -- Insert payment (no ON CONFLICT — pre-check handles idempotency)
...
-- Fix core_update_customer tags cast
...
BEGIN
  ...
  UPDATE public.customers SET
    ...
    tags=CASE WHEN v_update_fields ? 'tags' THEN ARRAY(SELECT jsonb_array_elements_text(v_update_fields->'tags')) ELSE tags END,
